...
function Client(conf) {
    ...
    this.files = this.parseGroupFiles();
    this.built = 0;
...
Client.prototype.connect = function () {
    if (!this.socket) {
        this.socket = ioc.connect(this.conf.url, {
            'reconnect': false,
...
Client.prototype.onConnect = function () {
    console.log('CLIENT CONNECTED requesting build on {0}'.format(this.conf.target));
    var client = this;
    ...
    var build = this.build = new Build({
        ...
        name: this.conf.name,
        started: new Date()
    }, client, null, this.conf.target, files);
    build.id = client.id;
    extend(build.conf, this.conf);
    delete build.conf.agent;
    delete build.conf.serverInstance;
    build.conf.logs.push(new Msg(build, this, sig, Msg.info, "The build is requested on {2}", this.conf.target));

    client.socket.emit('register', {
        id: client.id,
        save: !!this.conf.location
    });

    client.socket.emit('register-build', build.serialize());
    this.log(build, Msg.info, 'Reading {2} file{3}...', files.length, files.length === 1 ? '' : 's');
    FileHelper.readFiles(files, 'the servers-side sister of the cordova build client', this.uploadFiles.bind(this));
};
 ...
 * @param {Object} [err] - error object or null
 */
Client.prototype.uploadFiles = function (err) {
    if (err) {
        this.log(this.build, Msg.error, 'Error reading the input files\n{0}'.format(err));
        this.socket.emit('fail-build', this.build.serialize());
    ...
    try {
        ...
        if (size) {
            this.log(this.build, Msg.info, 'Uploading files to cordova build server...{2}', Common.getFilesize(size));
        }
        ...
        FileHelper.freeMemFiles(this.files);
    }
};
...
Client.prototype.onBuildFailed = function () {
    var client = this;
    if (++client.built >= client.conf.target.length) {
        client.disconnect();
    }
};
...
Client.prototype.done = function (err) {
    ...
    if (++this.built >= this.conf.target.length) {
        this.disconnect();
    }
};
...
Client.prototype.log = function (build, priority, message, args) {
    if (/Command failed/i.test(message)) {
        var e = new Error("client worker stack");
        message += e.stack;
    }
...
/**
 * Parse the groupfiles (according to conf.target)
 */
Client.prototype.parseGroupFiles = function () {
    var conf = this.conf;
    var groups = ['file'].concat(conf.target);
    var files = [];
    groups.forEach(function (group, isGroup) {
        if (group in conf) {
            if (typeof conf[group] == 'string' || conf[group] instanceof String) {
